...
export async function createGame(
  ...
  const result = await db.prepare(`
    INSERT INTO games (title, released, companie, poster, trailer, artworks, genre, estado, logros_obt, logros_total, console_pc, igdb_id, first_year_played, description, rating_metacritic, rating_opencritic, is_demo, is_early_access, is_testing)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
...
export async function updateGame(
  ...
  const allowedColumns = new Set([
    ...
    'first_year_played', 'description', 'rating_metacritic', 'rating_opencritic',
